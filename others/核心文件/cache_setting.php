	...
	foreach(C::t('common_nav')->fetch_all_mainnav() as $nav) {
		...
		if(!($nav['identifier'] == 5 && $nav['type'] == 0)) {
			foreach(C::t('common_nav')->fetch_all_subnav($nav['id']) as $subnav) {
				$item = "<a href=\"$subnav[url]\" hidefocus=\"true\" ".($subnav['title'] ? "title=\"$subnav[title]\" " : '').($subnav['target'] == 1 ? "target=\"_blank\" " : '').parsehighlight($subnav['highlight'])."><b>$subnav[name]<em></em></b></a>";
